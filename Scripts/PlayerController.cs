...
public class PlayerController : MonoBehaviour
{
    ...
    public Material player, player2, player3;
    ...
    void Start()
    {
        ...
        //mt = Resources.Load("Crazyball-game-template/Textures/Player/Materials/player2.mat", typeof(Material)) as Material;
    ...
    void Update()
    {
        if ((GaugeManager.val >= 0) && (GaugeManager.val <= 0.5))
        {
            GetComponent<MeshRenderer>().material = player;
        }


        if ((GaugeManager.val > 0.5) && (GaugeManager.val < 0.9))
        {
            GetComponent<MeshRenderer>().material = player2;
            followSpeedDelay = 0.4f;
        }

        if (GaugeManager.val >= 0.9)
        {
            GetComponent<MeshRenderer>().material = player3;
            followSpeedDelay = 0.8f;
        }
        ...
        if (!GameController.gameOver)
        {
            ...
            if (Application.isEditor)
            {
                ...
                transform.position = new Vector3(editorX, transform.position.y, -3.9f);
            }

            //set offset for ball
            transform.position = new Vector3(transform.position.x, 0.5f, -3.9f);
            ...
            if (transform.position.x > 10f)
                transform.position = new Vector3(10f, transform.position.y, -3.9f);
            if (transform.position.x < -10f)
                transform.position = new Vector3(-10f, transform.position.y, -3.9f);
    ...
    void touchControl()
    {
        if (Input.touchCount > 0 || Application.isEditor || Application.platform == RuntimePlatform.WindowsPlayer)
        {
            ...
            transform.position = new Vector3(touchX, transform.position.y, -3.9f);
        }
    }
